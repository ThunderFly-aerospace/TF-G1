...
difference(){
    ...
    for (i=[0:1]) rotate([0,0,180*i]){
        translate([bearing_efsm_17_width/2 + 2, 0, 0]) rotate([0, 90, 0]) cylinder(d=M8_screw_diameter, h=100, $fn=draft? 50:100);
        translate([bearing_efsm_17_width/2+ 2 + 5, -M8_nut_pocket/2, -M8_nut_diameter/2]) cube([M8_nut_height, M8_nut_pocket, 30]);
    }
