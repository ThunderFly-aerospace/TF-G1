       ...
       //color("Blue",0.5)
        ...
        difference (){ // profil závaží
            ...
            translate ([ -airfoil_depth/2 - weight_thickness, length/2, 0])
                rotate([90, 0, 0])
                    airfoil(naca = airfoil_NACA, L = airfoil_depth, N = 100, h = length, open = true); 
...
module 333_1035_B(twosided = true, draft = false){
    ...
    airfoil_NACA = 0014;    // typ použitého profilu
    airfoil_depth = 50; // hloubka profilu
    length = 60;       // celková délka polotovaru
    material_width = 70;
    thickness = 0.5;       // tloušťka jádra
    ...
    difference (){
        ...
        union(){
            translate([ -airfoil_depth/2 - weight_thickness + thickness, length/2, 0])
                rotate([90, 0, 0])
                    airfoil(naca = airfoil_NACA, L = airfoil_depth, N = 100, h = length, open = true); 
