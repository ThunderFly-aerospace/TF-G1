...
mirror([1,0,0])
   ...
   translate([-(width/2) - global_clearance/2, -depth + height/2-sqrt(2)*coupling_wall_thickness_undercarriage,height/2])  
	...
	translate([-width/2,0,height - height/4])
        rotate ([0,90,0])
            cylinder(h = width, r = M3_screw_diameter/2, $fn = draft ? 10 :20 );


	//nut
	translate([-width/2 - global_clearance, 0, height/4])
	   rotate([0,90,0])
	      cylinder (h = Nut_height_M3 + global_clearance, r = Nut_diameter_M3/2, $fn = 6);
    
    translate([width/2 - Nut_height_M3, 0, height/4])
	   rotate([0,90,0])
	       cylinder (h = Nut_height_M3 + global_clearance, r = Nut_diameter_M3/2, $fn = 6);

	translate([-width/2 - global_clearance, 0, height - height/4])
	   rotate([0,90,0])
	      cylinder (h = Nut_height_M3 + global_clearance, r = Nut_diameter_M3/2, $fn = 6);
    
    translate([width/2 - Nut_height_M3, 0, height - height/4])
	   rotate([0,90,0])
	       cylinder (h = Nut_height_M3 + global_clearance, r = Nut_diameter_M3/2, $fn = 6);
...
depth = tube_for_undercarriage_outer_diameter + M3_screw_diameter + 2*coupling_wall_thickness_undercarriage + thickness_between_tubes_undercarriage;
height = tube_for_undercarriage_outer_diameter + 2*coupling_wall_thickness_undercarriage;
...
difference(){
    ...
    translate([-width/2 - global_clearance/2,0,height/4])
        rotate ([0,90,0])
            cylinder(h = width + global_clearance, r = M3_screw_diameter/2, $fn = draft ? 10 :20 );

	translate([-width/2 - global_clearance/2,0,height - height/4])
        rotate ([0,90,0])
            cylinder(h = width+global_clearance, r = M3_screw_diameter/2, $fn = draft ? 10 :20 );
